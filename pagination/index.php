<html>
    ...
    <body>
        ...
        mysql_select_db('huntercollector');

        $pagename = $_SERVER['SCRIPT_NAME'];


        $sql = "SELECT * FROM aqbae_mymessages";
        $res = mysql_query($sql);
...
// pagination var end
// show result
        $result = mysql_query("SELECT * FROM aqbae_mymessages $max");
        $x = 1;
        while ($data = mysql_fetch_object($result)) {
            echo (($page_rows * ($pagenum - 1)) + $x) . ". " . $data->subject . "<br/>";
            $x++;
        }
